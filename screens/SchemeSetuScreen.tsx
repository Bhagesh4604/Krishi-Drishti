...
const SchemeSetuScreen: React.FC<SchemeSetuScreenProps> = ({ navigateTo, user, t }) => {
  ...
  const fetchExplanation = async (scheme: Scheme) => {
    ...
    try {
      ...
      Farmer Profile: ${user?.landSize} acres, Location: ${user?.district}, Category: ${user?.category}, Farming Type: ${user?.farmingType}, Crops: ${user?.crops.join(', ')}.
    ...
    <div className="h-full bg-[#F8FAF8] flex flex-col relative overflow-hidden">
           ...
           <div className="flex justify-between items-center relative z-10">
              ...
              <div className="text-right flex-1 ml-6">
                 ...
                 <div className="flex flex-wrap gap-1 justify-end">
                    ...
                    <span className="text-[8px] font-bold px-2 py-1 bg-white/10 rounded-lg">{user?.landSize} Acres</span>
                    <span className="text-[8px] font-bold px-2 py-1 bg-white/10 rounded-lg">{user?.farmingType}</span>
                 </div>
              </div>
           </div>
      ...
      {selectedScheme && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-end justify-center p-0 animate-in fade-in duration-300">
           <div className="w-full bg-white rounded-t-[3.5rem] max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-300">
              <div className="sticky top-0 bg-white p-8 pb-4 flex justify-between items-center border-b border-gray-50 z-10">
                 <div>
                    <h3 className="text-2xl font-black text-gray-900 leading-tight">{selectedScheme.name}</h3>
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mt-1">Application Hub</p>
                 </div>
                 <button onClick={() => setSelectedScheme(null)} className="p-3 bg-gray-50 rounded-2xl text-gray-400"><X size={24} /></button>
              </div>
              ...
              <div className="p-8 space-y-8 pb-20">
                 ...
                 <div className="pt-4 space-y-3">
                    <button className="w-full py-5 bg-green-700 text-white rounded-[2rem] font-black text-sm shadow-2xl shadow-green-100 flex items-center justify-center gap-3">
